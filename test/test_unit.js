...
"arithmetic operators": function(){
    ...
    test.expectError("r1 := r1 DIV r1", "operator 'DIV' type mismatch: INTEGER expected, got 'REAL'");
    test.parse("r1 := r1 + r2");
    test.parse("r1 := r1 - r2");
    test.parse("r1 := r1 * r2");
    test.parse("r1 := r1 / r2");

    test.expectError("b1 := b1 + b1", "operator '+' type mismatch: numeric type expected, got 'BOOLEAN'");
    test.expectError("c1 := c1 - c1", "operator '-' type mismatch: numeric type expected, got 'CHAR'");
    test.expectError("p1 := p1 * p1", "operator '*' type mismatch: numeric type expected, got 'PROCEDURE'");
    test.expectError("ptr1 := ptr1 / ptr1", "operator '/' type mismatch: numeric type expected, got 'POINTER TO anonymous RECORD'");
    test.expectError("s1 := +s1", "operator '+' type mismatch: numeric type expected, got 'SET'");
    test.expectError("b1 := -b1", "operator '-' type mismatch: numeric type expected, got 'BOOLEAN'");
    test.expectError("s1 := +b1", "operator '+' type mismatch: numeric type expected, got 'BOOLEAN'");
},
